...
class MultipleBinaryClassification(tasks.Task, core.Configurable):
    ...
    def forward(self, batch):
        ...
        pred, output_graph_features = self.predict(batch, all_loss, metric)
        target = self.target(batch)
        ...
        return all_loss, metric, output_graph_features
...
class NodePropertyPrediction(tasks.Task, core.Configurable):
    ...
    def forward(self, batch):
        ...
        (pred, output_rep), target = self.predict_and_target(batch, all_loss, metric)
        ...
        return all_loss, metric, output_rep
