...
class Engine(core.Configurable):
    ...
    def evaluate(self, split, log=True):
        ...
        for batch in dataloader:
            if self.device.type == "cuda":
                batch = utils.cuda(batch, device=self.device)
            (pred, output_rep), target = model.predict_and_target(batch)
            #pred, target, output_rep = model.predict_and_target(batch)
            preds.append(pred)
            targets.append(target)
            output_rep_list.append(output_rep)

        pred = utils.cat(preds)
        target = utils.cat(targets)
        output_rep_list = utils.cat(output_rep_list)
        if self.world_size > 1:
            pred = comm.cat(pred)
        ...
        return metric, pred, target, output_rep_list
